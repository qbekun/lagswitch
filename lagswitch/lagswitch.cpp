...
void UpdateDiscordRPC(int secondsLeft = 0, UINT64 blockedPackets = 0) {
    ...
    std::vector<std::string> statePool = {
        "断線中 // signal: unstable",
        "link lost… awaiting null",
        "wired ghost detected",
        "誰もいない — no one is here",
        "network echo: empty",
    ...
    std::vector<std::string> detailPool = {
        "時間残り: " + std::to_string(secondsLeft) + " 秒 | noise shifting",
    ...
    presence.largeImageText = "n0ise断線 // disconnect from the real";
    presence.smallImageKey = "断線";
...
void UpdateConsoleTitle() {
    std::string title = "n0ise断線 [lolikuza.ovh] - STATUS: ";
